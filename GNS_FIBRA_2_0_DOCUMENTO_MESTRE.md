...
Atualização em 19/07/2026: schema, seed e migrations deixaram de fixar o nome do banco e agora operam exclusivamente no banco selecionado pelo cliente MySQL. O schema mantém 13 tabelas sem apagar dados; o seed é transacional e limitado à instalação inicial marcada pela ausência de `company_name`; migrations não repetem backfills nem sobrescrevem edições administrativas. O fluxo foi validado duas vezes no banco local e duas vezes em banco descartável, com backup e hashes determinísticos. A operação completa está documentada em `database/MIGRATIONS-README.md`.

Atualização em 19/07/2026: o módulo Banners foi consolidado como “Imagens do Hero”. Identificação e observação são metadados internos; a API pública entrega apenas id, caminho e ordem da imagem. O título do Hero continua em `settings.hero_title`, a descrição em `settings.about_text` e os CTAs permanecem no componente, sem fontes concorrentes. A Cobertura passou a renderizar e pesquisar também `coverage.description`, preservando links, ordem, estado vazio e responsividade.

Atualização em 19/07/2026: a logo pública passou a ser administrável por `settings.company_logo_path`, com uploads seguros em `uploads/branding/`. Header e Footer usam `config.company.logoUrl` por um componente compartilhado; ausência, caminho rejeitado ou erro de carregamento restauram `public/logo-gns.png` sem loop. A logo oficial permanece versionada e imutável. Favicon, Open Graph, login e sidebar administrativa continuam independentes, e não houve schema, seed ou migration.
